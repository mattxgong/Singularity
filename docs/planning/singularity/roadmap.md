...
- A starfield prototype has measured frame cost on a throttled profile against the 2 millisecond budget, and the verdict governs `SINGULARITY-026`. The verdict applies to main-thread animation; compositor-driven CSS animation falls outside it.
...
- The decorative layer renders on every viewport and in both themes, adds no client JavaScript, and confines its animation to `transform` and `opacity`.
...
- Set the operating system to reduced motion. Confirm that no transition exceeds a perceptible duration, and that the decorative layer stands completely still.
