...
| AS-4  | A Canvas 2D starfield holds 2ms per frame on a mid-tier device. `Falsified` on 2026-09-17 | Ship only the static SVG. The design does not depend on animation. Executed             | SINGULARITY-005                  |
...
Mitigation: D4 capped the ceiling at Canvas 2D and `SINGULARITY-005` measured before committing. The measurement failed at 6 times throttling, so `SINGULARITY-026` ships only the static SVG and the risk is closed by removal rather than by budget enforcement. No animation module reaches the client. `SINGULARITY-058` still enforces the performance budget in continuous integration so a later regression fails the build.
...
| D4  | Motion ceiling was Canvas 2D; superseded by the static SVG on measured evidence               | Product brief, P0 risk analysis, and self-reported `prakhau143/Portfolio` measurements | The reference reports a performance score of 64 and a 33.5-second Largest Contentful Paint for its heavy-media alternative                                           | None. The static SVG fallback shipped as designed                      | Confirmed    | 2026-09-19   |
