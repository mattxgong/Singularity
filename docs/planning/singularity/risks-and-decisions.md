...
| AS-4  | A Canvas 2D starfield holds 2ms per frame on a mid-tier device. `Falsified` on 2026-09-17 | Ship the SVG layer without main-thread animation. Executed, then amended on 2026-09-19 to add compositor-only drift | SINGULARITY-005                  |
...
Mitigation: D4 capped the ceiling at Canvas 2D and `SINGULARITY-005` measured before committing. The measurement failed at 6 times throttling, so the canvas was withdrawn. What `SINGULARITY-005` falsified was main-thread animation, and the amended D4 permits only compositor-driven CSS animation of `transform` and `opacity`, which consumes none of that budget. No animation module reaches the client, and the shipped decorative layer is still zero kilobytes of JavaScript. `SINGULARITY-058` enforces the performance budget in continuous integration so a later regression fails the build.

Early warning: any proposal to add an animation library, a video, or a WebGL scene, or any decorative animation touching a property outside `transform` and `opacity`.
...
| D4  | Motion ceiling is compositor-only CSS; Canvas 2D and WebGL both rejected on measured evidence | Product brief, P0 risk analysis, self-reported `prakhau143/Portfolio` measurements, and the `SINGULARITY-005` traces | Main-thread animation missed its frame budget and the WebGL reference reports a performance score of 64 with a 33.5-second Largest Contentful Paint; compositor animation costs no main-thread time and no JavaScript | Low. Deleting one keyframe block restores the fully static layer       | Confirmed    | 2026-09-19   |
...
| A5  | Starfield as a server-rendered layer, drifting on the compositor | Canvas client leaf, client wrapper, or WebGL scene | Drift shows measurable main-thread cost or a Lighthouse regression      |
...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| WebGL and Three.js              | D4. The performance cost is quantified by two reference projects                                                          | Never, absent a documented requirement                                           |
| Video backgrounds               | 97 MB of media in the reference project is indefensible                                                                   | Never                                                                            |
| Motion or GSAP                  | CSS transitions plus the compositor-driven starfield cover the need                                                       | A specific interaction requires physics                                          |
